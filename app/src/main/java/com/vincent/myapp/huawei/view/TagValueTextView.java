...
public class TagValueTextView extends View {
    ...
    public TagValueTextView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
...
//            w = tagRect.width() + valueRect.width() + mMargin + DpUtil.dp2px(mContext,5);  //单位是px
            w = (int) (mValuePaint.measureText(mValueString) + mTagPaint.measureText(tag) + mMargin);  //单位是px
            h = valueRect.height()+DpUtil.dp2px(mContext,5);
        } else if (widthSpecMode == MeasureSpec.AT_MOST) {
            //只要宽度布局参数为wrap_content， 宽度给固定值200dp(处理方式不一，按照需求来)
            w =  (int) (mValuePaint.measureText(mValueString) + mTagPaint.measureText(tag) + mMargin);
