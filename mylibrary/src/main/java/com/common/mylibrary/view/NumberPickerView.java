...
public class NumberPickerView extends View{
    ...
    // default text color of not selected item 未选中项的默认文本颜色
    private static final int DEFAULT_TEXT_COLOR_NORMAL = 0XFF333333;

    // default text color of selected item 选定项目的默认文本颜色
    private static final int DEFAULT_TEXT_COLOR_SELECTED = 0XFFF56313;

    // default text size of normal item 普通项的默认文本大小
    private static final int DEFAULT_TEXT_SIZE_NORMAL_SP = 14;

    // default text size of selected item 选定项的默认文本大小
    private static final int DEFAULT_TEXT_SIZE_SELECTED_SP = 16;

    // default text size of hint text, the middle item's right text 默认的文本大小提示文本，中间项目的正确文本
    private static final int DEFAULT_TEXT_SIZE_HINT_SP = 14;

    // distance between selected text and hint text 选定文本与提示文本之间的距离
    private static final int DEFAULT_MARGIN_START_OF_HINT_DP = 8;

    // distance between hint text and right of this view, used in wrap_content mode 提示文本与视图右侧之间的距离，用于wrap_content模式
    private static final int DEFAULT_MARGIN_END_OF_HINT_DP = 8;

    // default divider's color  默认的分频器的颜色
    private static final int DEFAULT_DIVIDER_COLOR = 0XFFF56313;

    // default divider's height  默认的分频器的高度
    private static final int DEFAULT_DIVIDER_HEIGHT = 2;

    // default divider's margin to the left & right of this view 这个视图的左边和右边的默认分隔符的边距
    private static final int DEFAULT_DIVIDER_MARGIN_HORIZONTAL = 0;

    // default shown items' count, now we display 3 items, the 2nd one is selected 默认显示项目的计数，现在我们显示3个项目，选择第二个
    private static final int DEFAULT_SHOW_COUNT = 3;

    // default items' horizontal padding, left padding and right padding are both 5dp, 默认项的水平填充、左填充和右填充都是5dp，
    // only used in wrap_content mode 仅在wrap_content模式下使用
    private static final int DEFAULT_ITEM_PADDING_DP_H = 5;

    // default items' vertical padding, top padding and bottom padding are both 2dp, 默认项目的垂直填充、顶部填充和底部填充都是2dp，
    ...
    private static final int HANDLER_INTERVAL_REFRESH = 36;//millisecond

    // in millisecond unit, default duration of scrolling an item' distance 在毫秒单元中，滚动项目距离的默认持续时间
    private static final int DEFAULT_INTERVAL_REVISE_DURATION = 300;

    // max and min durations when scrolling from one value to another 从一个值滚动到另一个值时的最大和最小持续时间
    ...
    private static final boolean DEFAULT_SHOW_DIVIDER = true;//默认显示分割线
    private static final boolean DEFAULT_WRAP_SELECTOR_WHEEL = true;
    private static final boolean DEFAULT_CURRENT_ITEM_INDEX_EFFECT = false;//默认选中当前下标
